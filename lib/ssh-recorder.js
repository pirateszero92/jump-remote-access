...
async function rebuildIndex(baseDir) {
  const sessions = [];
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            if (stat.isFile()) {
              ...
              sessions.push(summarizeSession(meta));
              continue;
            }
          } catch {
            // No meta.json in this directory, look deeper
          }
...
async function readIndex(baseDir) {
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      console.warn(`[SSH-RECORD] index.json is corrupted (${error.message}). Rebuilding...`);
      try {
        const rebuilt = await rebuildIndex(baseDir);
        await writeIndex(baseDir, rebuilt);
        return rebuilt;
      } catch (rebuildError) {
        console.error(`[SSH-RECORD] Failed to rebuild index:`, rebuildError);
        return { sessions: [] };
      }
    }
...
async function writeIndex(baseDir, index) {
  ...
  await fsp.writeFile(tempPath, `${JSON.stringify(index, null, 2)}\n`, 'utf8');
...
async function deleteSession(baseDir, sessionId) {
  const found = await findSession(baseDir, sessionId);

  if (found) {
    try {
      await fsp.rm(found.sessionDir, { recursive: true, force: true });
    } catch (error) {
      console.error(`[SSH-RECORD] Failed to delete directory: ${found.sessionDir}`, error);
    }
  }

  const index = await readIndex(baseDir);
  const exists = index.sessions.some((entry) => entry.id === sessionId);
  if (!exists && !found) {
    return false;
  }

  index.sessions = index.sessions.filter((entry) => entry.id !== sessionId);
  await writeIndex(baseDir, index);
  return true;
}
