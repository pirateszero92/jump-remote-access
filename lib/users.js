...
async function verifyPassword(password, storedHash) {
  ...
  try {
    const actualBuffer = await scryptAsync(String(password), salt, 64);
    return crypto.timingSafeEqual(Buffer.from(expected, 'hex'), actualBuffer);
  } catch {
    return false;
  }
}
...
async function updateUser(userId, payload) {
  ...
  if (payload.password !== undefined && String(payload.password).length > 0) {
    ...
    current.passwordHash = await hashPassword(String(payload.password));
  }
