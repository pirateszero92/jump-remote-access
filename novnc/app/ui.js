 ...
 * ------v------*/
    ...
    async serverVerify(e) {
        ...
        if (type === 'RSA') {
            ...
            const autoApproveRaw = queryParams.get('auto_approve') ?? hashParams.get('auto_approve') ?? '';
            const autoApprove = ['1', 'true', 'yes', 'on'].includes(autoApproveRaw.toLowerCase());

            if (autoApprove) {
                Log.Warn("Auto-approving VNC server identity.");
                UI.rfb.approveServer();
                return;
            }

            const publickey = e?.detail?.publickey;
            let fingerprint = _("Unavailable (requires HTTPS or localhost)");

            // SubtleCrypto can be unavailable on insecure origins (e.g. plain HTTP over IP).
            if (window.crypto?.subtle?.digest && publickey) {
                fingerprint = await window.crypto.subtle.digest("SHA-1", publickey);
